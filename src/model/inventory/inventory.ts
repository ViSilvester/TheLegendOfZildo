...
export class Inventory {
    ...
    private lifePotion = 20;
    private magicPotion = 20;
    private powerPotion = 0;
    ...
    //keybordTogle
    virtualKeyboard = new Map<string, string>();


    //Ui data
    status = "inventory";
    transitionTimer = 0;
    inventoryPos = new Vec2(1, 1)
    inventoryDim = new Vec2(18, 20);
    sel = 0;
    game_saved_note = 0;
    ...
    drinkPotion(player: Player, potionId: number) {
        switch (potionId) {
            case 1:
    ...
    update(game: Game) {
        ...
        if (this.transitionTimer == 20) {
            ...
            if (this.status == "inventory") {
                if (this.sel < 7) { this.sel = 7; }
            }
            if (this.status == "confirm") {
                if (this.sel < 5) { this.sel = 5; }
                if (this.sel > 6) { this.sel = 6; }
            }
            else if (this.status == "lvlUp") {
                if (this.sel > 4) { this.sel = 4; }
            }

            if (this.game_saved_note > 0) { this.game_saved_note--; }
        }
    }

    updateKeyboardinput(game: Game) {
        ...
        if (this.status == "lvlUp") {
            ...
            if (bonus != 0) {
                ...
                if (this.inteligence < 0) { this.inteligence = 0 };
                if (this.magic_power < 0) { this.magic_power = 0 };
            }
        }
    }


    renderInventory(game: Game) {
        ...
        var line = new Vec2(
            (this.inventoryPos.x + 1) * game.tilesize,
            (this.inventoryPos.y + 1) * game.tilesize
        );
    ...
    renderMoldura(game: Game, maxSize: number) {
    ...
    renderStatusBar(game: Game) {
        ...
        game.draw.drawImage(
            game.ui_texture,
            new Vec2(game.tilesize * 10, game.tilesize * 1.5),
            new Vec2(game.tilesize * ui_scale, game.tilesize * ui_scale),
            new Vec2(0, 0),
            new Vec2(16, 16)
        );
        game.draw.drawImage(
            game.ui_texture,
            new Vec2(game.tilesize * 12.5, game.tilesize * 1.5),
            new Vec2(game.tilesize * ui_scale, game.tilesize * ui_scale),
            new Vec2(16, ki_1 * 16),
            new Vec2(16, 16)
        );
        game.draw.drawImage(
            game.ui_texture,
            new Vec2(game.tilesize * 15, game.tilesize * 1.5),
            new Vec2(game.tilesize * ui_scale, game.tilesize * ui_scale),
            new Vec2(32, ki_2 * 16),
            new Vec2(16, 16)
        );
        game.draw.drawImage(
            game.ui_texture,
            new Vec2(game.tilesize * 17.5, game.tilesize * 1.5),
            new Vec2(game.tilesize * ui_scale, game.tilesize * ui_scale),
            new Vec2(48, ki_3 * 16),
            new Vec2(16, 16)
        );
        game.draw.drawImage(
            game.ui_texture,
            new Vec2(game.tilesize * 20, game.tilesize * 1.5),
            new Vec2(game.tilesize * ui_scale, game.tilesize * ui_scale),
            new Vec2(64, ki_4 * 16),
            new Vec2(16, 16)
        );
